function problemObject = setProblemObject_v3_normalized(group_description,group_number,output_path)
    ...
    % Normalize each data group by 1/max(GFP/broc_time_course) 
        % Initialize weights 
    normalization_weights = nan(height(selected_table),1); 
        % Get normalization for each group 
    updated_group_num = unique(selected_table.Group); 
    updated_group_num_height = nan(length(updated_group_num),1);
    for updated_group_idx = 1:length(updated_group_num)
        group_idx_list = find(selected_table.Group == updated_group_num(updated_group_idx)); 
        updated_group_num_height(updated_group_idx) = length(group_idx_list); 
        reporter_time_course = selected_table.GFP_concentration(group_idx_list); 
        normalization_weights(group_idx_list) = 1 ./ max(reporter_time_course); 
    end

    % Add to problemObject
    problemObject.Weights = normalization_weights; 
