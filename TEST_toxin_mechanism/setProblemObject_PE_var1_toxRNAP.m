function problemObject = setProblemObject_PE_var1_toxRNAP(group_description,output_path,weight,error_model,short_time_course)
    ...
    for rxn_name_idx = 1:length(model_reaction_name_list)
        ...
        if contains(rxn_name_lhs,'CUTP:AGTP:') && ~contains(rxn_name_rhs,'term')
            tx_elong_rxn_name_list{end + 1} = reaction_name;
        end
    end
    ...
    for tx_rxn_idx = 1:length(tx_elong_rxn_name_list)
        tx_rxn_obj = sbioselect(Mobj,'Type','Reaction','Reaction',tx_elong_rxn_name_list{tx_rxn_idx}); 
    ...
    set(toxin_removal, 'ReactionRate', 'tx_capacity_param');
        % Add a reaction for RNAP degradation 
    RNAP_deg = addreaction(Mobj,'RNAP -> null');
    set(RNAP_deg,'ReactionRate','k_toxin * (1 + tanh(toxin - toxin_threshold)) * RNAP')

    t7RNAP_deg = addreaction(Mobj,'t7RNAP -> null');
    set(t7RNAP_deg,'ReactionRate','k_toxin * (1 + tanh(toxin - toxin_threshold)) * t7RNAP')
