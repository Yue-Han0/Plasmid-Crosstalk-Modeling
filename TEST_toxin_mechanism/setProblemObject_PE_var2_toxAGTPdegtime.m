function problemObject = setProblemObject_PE(group_description,output_path,weight,error_model,short_time_course)
    ...
    %%%%%%%%%short time course%%%%%%%%%
        %Delete any row with Time > 10800 
    if exist('short_time_course','var') && isequal(short_time_course,1)
        keep_idx = selected_table.Time < 10800; 
        selected_table = selected_table(keep_idx,:); 
    end
    %%%%%%%%%%%%%%%%%%

    %%%%%%add weights/change error model%%%%%%
    % Error model 
    if exist('error_model','var') && ~isempty(error_model)
        problemObject.ErrorModel = error_model; 
    end

    % Apply to protein expression data fitting 
    if ~isempty(weight)
        if ~isempty(PE_selected_table)
            PE_weights = ones(height(PE_selected_table),1); 
            % For each 'time iteration', pick t = 10800. If 10800 doesn't exist,
            % pick the most adjacent timepoint 
                % Identify start of iteration 
            timepoint_off_1 = [1;PE_selected_table.Time(1:end-1)]; 
            time_difference = PE_selected_table.Time - timepoint_off_1; 
            start_point = find(time_difference < 0); 
            PE_emphasis_idx = []; 
            for time_start_idx = 1:length(start_point)
                if ~isequal(time_start_idx,length(start_point))
                    time_interval = PE_selected_table.Time(start_point(time_start_idx):start_point(time_start_idx + 1)-1); 
                else
                    time_interval = PE_selected_table.Time(start_point(time_start_idx):end); 
                end
                time_interval_diff = abs(time_interval - 10800); 
                [~,time_add_idx] = min(time_interval_diff); 
                PE_emphasis_idx = [PE_emphasis_idx;start_point(time_start_idx) + time_add_idx - 1];
            end
            PE_weights(PE_emphasis_idx) = weight; 
        else
            PE_weights = []; 
        end
        % Apply to transcription-level data fitting 
        if ~isempty(tx_selected_table)
            tx_weights = ones(height(tx_selected_table),1); 
            % For broc data, emphasize on the max point in each iteration 
            timepoint_off_1 = [1;tx_selected_table.Time(1:end-1)]; 
            time_difference = tx_selected_table.Time - timepoint_off_1; 
            start_point = find(time_difference < 0); 
            tx_emphasis_idx = []; 
            for time_start_idx = 1:length(start_point)
                if ~isequal(time_start_idx,length(start_point))
                    data_interval = tx_selected_table.mRNA_concentration(start_point(time_start_idx):start_point(time_start_idx + 1)-1); 
                else
                    data_interval = tx_selected_table.mRNA_concentration(start_point(time_start_idx):end); 
                end
                [~,max_idx] = max(data_interval); 
                tx_emphasis_idx = [tx_emphasis_idx;start_point(time_start_idx) + max_idx - 1];
            end
            tx_weights(tx_emphasis_idx) = weight; 
        else
            tx_weights = [];
        end
        if isempty(PE_weights) || isempty(tx_weights)
            problemObject.Weights = [tx_weights;PE_weights]; 
        else
            problemObject.Weights = ones(length([tx_weights;PE_weights]),2); 
            problemObject.Weights(1:length(tx_weights),1) = tx_weights; 
            problemObject.Weights(length(tx_weights) + 1:end,2) = PE_weights; 
        end
    end
    ...
    for rxn_idx = 1:length(Mobj_rxns)
        ...
        for rxn_param_idx = 1:length(rxn_params)
            ...
            if contains(rxn_param_name,'TXTL_RNAdeg_R') && contains(rxn.Reaction,'sfGFP')
    ...
    % Taken transcription toxicity into account 
        % Add new parameters named 'AGTPdeg_time_init',
        % 'tx_capacity', 'k_toxin' 
    AGTP_deg_time_0 = addparameter(Mobj,'AGTP_deg_time_0'); 
    ...
    for tx_rxn_idx = 1:length(tx_reaction_name_list)
        tx_rxn_obj = sbioselect(Mobj,'Type','Reaction','Reaction',tx_reaction_name_list{tx_rxn_idx}); 
        addproduct(tx_rxn_obj,'toxin',1); 
    end
        % Add a reaction for toxin buffering
    toxin_removal = addreaction(Mobj,'toxin -> null'); 
    % toxin_removal_kineticLaw = addkineticlaw(toxin_removal, 'Henri-Michaelis-Menten');
    % toxin_removal_Vmax = addparameter(toxin_removal_kineticLaw, 'tx_capacity_param');
    % toxin_removal_Km = addparameter(toxin_removal_kineticLaw,'Km',1e-12); 
    % set(toxin_removal_kineticLaw, ParameterVariableName, {'tx_capacity_param','Km'});
    % set(toxin_removal, 'ReactionRate', 'tx_capacity_param * [toxin] /(1e-12 + [toxin])');
    set(toxin_removal, 'ReactionRate', 'tx_capacity_param');

    AGTPdeg_time_param = sbioselect(Mobj.Parameters,'Name','AGTPdeg_time'); 
    AGTPdeg_time_param.Constant = false; 

    % addrule(Mobj,'AGTPdeg_time = AGTP_deg_time_0','initialAssignment'); 
    addrule(Mobj,'AGTPdeg_time = AGTP_deg_time_0 - k_toxin * toxin * (1 + tanh(toxin - toxin_threshold))','repeatedAssignment'); 
